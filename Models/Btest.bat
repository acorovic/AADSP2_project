...
for %%f in (..\TestStreams\*.*) do ( 
	...
	echo     model 0
    "%CONFIGURATION%\model0.exe" "%%f" "OutStreams//%%~nf_model0.wav" "%CHANNEL_NUM%" "%DISTORTION_OPTION%" 
	echo     model 1
	"%CONFIGURATION%\model1.exe" "%%f" "OutStreams//%%~nf_model1.wav" "%CHANNEL_NUM%" "%DISTORTION_OPTION%" 
	echo     model 2
	"%CONFIGURATION%\model2.exe" "%%f" "OutStreams//%%~nf_model2.wav" "%CHANNEL_NUM%" "%DISTORTION_OPTION%" 
	...
	(
		...
		@echo ^<CL_PROJECT^>
		@echo     ^<argv^>%%f OutStreams\%%~nf_model3.wav %CHANNEL_NUM% %DISTORTION_OPTION%^</argv^>
